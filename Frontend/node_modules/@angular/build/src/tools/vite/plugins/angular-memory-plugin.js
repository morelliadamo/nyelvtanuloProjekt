...
async function loadViteClientCode(file, disableViteTransport = false) {
    const originalContents = await (0, promises_1.readFile)(file, 'utf-8');
        // Try to remove the Vite overlay suggestion text from the client.
        // The Vite client shape can vary between versions; if the exact
        // substring isn't found, fall back to the original contents instead
        // of throwing an assertion error. This keeps the dev server running
        // even if the plugin's patch can't be applied to the client code.
        ...
        if (originalContents === updatedContents) {
            // No replacement performed; skip the assertion and continue with the original
            // contents to avoid crashing on Vite client changes.
    ...
    if (disableViteTransport) {
        ...
        // If the expected pattern changed in the client, don't throw; just continue.
        if (previousUpdatedContents === updatedContents) {
          // no-op: pattern wasn't found, but we won't fail.
        }
